...
namespace SpawnDev.BlazorJS.BrowserExtension.JSObjects
{
    public class StorageArea : JSObject
    {
        ...
        public async Task<T> Get<T>(string key)
        {
            ...
            if (tmp == null || tmp.JSRef!.PropertyIsUndefined(key)) return default(T);
            return tmp.JSRef!.Get<T>(key);
        }
        public async Task<T> Get<T>(string key, T defaultValue)
        {
            using var tmp = await JSRef.CallAsync<JSObject?>("get", key);
            if (tmp == null || tmp.JSRef!.PropertyIsUndefined(key)) return defaultValue;
            return tmp.JSRef!.Get<T>(key);
        }
        public async Task<bool> Exists(string key)
        {
            using var tmp = await JSRef.CallAsync<JSObject?>("get", key);
            return tmp != null && !tmp.JSRef!.PropertyIsUndefined(key);
        }
